...
namespace Niobium.Identity.API
{
        ...
        ILogger<Profile> logger)
    {
        ...
        {
            bool result = await this.CheckClientTokenAsync(token, tenant, user, cancellationToken);
            return result || await this.CheckServicePrincipalAsync(token, tenant, user, cancellationToken);
        }
        ...
        private async Task<bool> CheckClientTokenAsync(string token, Guid tenant, Guid user, CancellationToken cancellationToken)
        {
            try
            {
                ...
                return principal != null && principal.TryGetClaim<Guid>(ClaimTypes.Sid, out Guid u) && u == user
        ...
        private async Task<bool> CheckServicePrincipalAsync(string token, Guid tenant, Guid user, CancellationToken cancellationToken)
        {
            ...
            JsonWebKeySet keys = await this.GetSigningKeysAsync(options.Value.TenantID.Value, cancellationToken);
